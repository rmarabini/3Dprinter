...
difference()
{
  ...
  translate([-0, -0, -0. + h/2])
      cube([2*r,2*r,h], center=true);
        translate([r-1,-r+1,h/2.+2])
      difference(){
      cube([2.5*r,ry,h-4], center=true);
      cube([2.5*r-2,ry,h-6], center=true);          
      }
            
    translate([r+2.5-1,-r+1,h*2/4.+2])
    cube([r+1,ry,1], center=true);
    translate([r+2.5-1,-r+1,h*1/4.+3])
    cube([r+1,ry,1], center=true);
...
screw_standard = [
  ...
  [
  // M1_6   M2     M2_5   M3     M3_5   M4     M5     M6     M8     M10
   [ 3.0,   3.8,   4.7,   6.5,   6.5,   7.5,   9.2,  11.0,  14.5,  18.0 ], // dk
   [ 0.96,  1.2,   1.5,   2.0,  1.93,  2.2,   2.5,   3.0,   4.0,   5.0 ]  // k
  ],
